...
if command -v opengrep &> /dev/null; then
    ...
    opengrep scan --config auto --quiet --json --output opengrep-results.json crates/anno/ tests/ examples/ 2>/dev/null || echo '{"results":[]}' > opengrep-results.json
    ...
    for rule_file in .opengrep/rules/*.yaml; do
        if [ -f "$rule_file" ]; then
            ...
            count=$(opengrep scan -f "$rule_file" --quiet --json crates/anno/ 2>/dev/null | jq '.results | length' || echo "0")
