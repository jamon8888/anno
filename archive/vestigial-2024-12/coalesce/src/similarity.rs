...
//!
//! 1. **Preprocessing**: Unicode normalization (NFKC), case folding
...
//! │              ByteEmbedder (future)                          │
//! ├─────────────────────────────────────────────────────────────┤
//! │  - Tokenizer-free (operates on raw bytes)                   │
//! │  - Contrastive learning on entity pairs                     │
//! │  - Fast inference (small model, SIMD)                       │
//! │  - Multilingual by design (byte-level = script-agnostic)    │
//! └─────────────────────────────────────────────────────────────┘
//! ```
//!
//! ## Research Directions
...
//!
//! ```rust
//! use anno_coalesce::similarity::{Similarity, Script};
//!
...
/// # Note
///
/// Currently uses a simplified normalization. For production use with
/// `unicode-normalization` crate, enable the `unicode-normalization` feature.
///
/// # TODO
///
/// - Add `unicode-normalization` crate for proper NFKC
/// - Add Han variant normalization (traditional ↔ simplified)
/// - Add phonetic normalization (Pinyin, Romaji)
pub fn normalize(s: &str) -> String {
    // Simplified normalization without external crate
    s.chars()
        .filter_map(|c| {
            if c.is_whitespace() {
                Some(' ')
...
/// Levenshtein edit distance (basic implementation).
///
/// For production use, consider the `strsim` or `rapidfuzz` crate.
///
/// # Complexity
///
...
/// Compute string similarity using a hybrid approach.
///
/// This is the main entry point, providing backward compatibility
/// with the old API while using the new multilingual pipeline.
///
