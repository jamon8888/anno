...
impl NERDataset {
    ...
    pub fn from_json<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let test_cases = crate::datasets::load_json_ner_dataset(path)?;
    ...
    pub fn from_conll<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let test_cases = crate::load_conll2003(path)?;
        let examples = test_cases
            .into_iter()
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let test_cases = crate::datasets::load_ner_dataset(path)?;
        let examples = test_cases
            .into_iter()
            .map(|(text, entities)| AnnotatedExample::new(text, entities))
            .collect();
        ...
        for ex in &self.examples {
            ...
            *difficulties.entry(ex.difficulty).or_insert(0) += 1;
            ...
            for entity in &ex.entities {
                let type_str = crate::entity_type_to_string(&entity.entity_type);
