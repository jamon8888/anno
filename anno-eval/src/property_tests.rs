...
// =========================================================================
// Morphology Round-Trip Properties
// =========================================================================

#[cfg(test)]
mod morphology_proptests {
    use crate::preprocess::morphology::{MorphologicalPreprocessor, SegmentationStrategy};
    use proptest::prelude::*;

    proptest! {
        #![proptest_config(ProptestConfig::with_cases(200))]

        /// Property: Character segmentation should preserve total length
        #[test]
        fn char_segmentation_preserves_length(text in "[a-zA-Z]{1,50}") {
            let preprocessor = MorphologicalPreprocessor::new()
                .with_strategy(SegmentationStrategy::Character);

            let result = preprocessor.segment(&text).unwrap();

            // Total morpheme characters should equal original
            let total_chars: usize = result.morphemes.iter()
                .map(|m| m.text.chars().count())
                .sum();

            prop_assert_eq!(total_chars, text.chars().count(),
                "Character count mismatch: {} vs {}", total_chars, text.chars().count());
        }

        /// Property: Rule-based segmentation should be reversible
        #[test]
        fn rule_segmentation_reversible(text in "[a-z]+-[a-z]+-[a-z]+") {
            let preprocessor = MorphologicalPreprocessor::new()
            ...
            // Reconstructing should give original (minus separators)
            let reconstructed: String = result.morphemes.iter()
                .map(|m| m.text.as_str())
                .collect::<Vec<_>>()
                .join("-");
        ...
        /// Property: Span mappings should be valid
        #[test]
        fn span_mappings_valid(text in "[a-z]{1,20}-[a-z]{1,20}") {
            let preprocessor = MorphologicalPreprocessor::new()
                .with_strategy(SegmentationStrategy::RuleBased {
                    boundary_chars: vec!['-'],
                });
        ...
        /// Property: Empty input produces empty morphemes
        #[test]
        fn empty_input_empty_morphemes(_seed in 0u32..100) {
            let preprocessor = MorphologicalPreprocessor::new()
                .with_strategy(SegmentationStrategy::Character);

            let result = preprocessor.segment("").unwrap();
            prop_assert!(result.morphemes.is_empty());
        }
    }
}
...
mod discontinuous_proptests {
    ...
    use crate::discontinuous::{DiscontinuousGold, DiscontinuousNERMetrics};
    use proptest::prelude::*;
