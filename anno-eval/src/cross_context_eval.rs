...
use crate::cdcr::{CrossDocCluster, Document};
use crate::cluster_encoder::{ClusterEncoder, MergeScorer};
use crate::coref::{CorefChain, Mention};
use crate::coref_metrics::{conll_f1, CorefScores};
use crate::neural_cluster_encoder::{
...
pub fn evaluate_cross_document<E: ClusterEncoder + Clone, S: MergeScorer + Clone>(
    ...
    let (muc_p, muc_r, muc_f1) = crate::coref_metrics::muc_score(&all_pred_chains, &all_gold_chains);
    let (b3_p, b3_r, b3_f1) = crate::coref_metrics::b_cubed_score(&all_pred_chains, &all_gold_chains);
    let (ceaf_p, ceaf_r, ceaf_f1) = crate::coref_metrics::ceaf_e_score(&all_pred_chains, &all_gold_chains);
    let (lea_p, lea_r, lea_f1) = crate::coref_metrics::lea_score(&all_pred_chains, &all_gold_chains);
...
pub fn evaluate_long_document<E: ClusterEncoder + Clone, S: MergeScorer + Clone>(
    ...
    for doc in documents {
        ...
        if windows.is_empty() {
            // If no windows, treat as single context
            all_gold_chains.extend(doc.gold_chains.clone());
            all_pred_chains.extend(doc.gold_chains.clone()); // Placeholder
            continue;
        }
...
mod tests {
    use super::*;
    use crate::cluster_encoder::{CosineMergeScorer, HeuristicClusterEncoder};
