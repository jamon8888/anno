...
use crate::loader::{DatasetId, DatasetLoader, LoadedDataset};
#[cfg(feature = "eval-profiling")]
use crate::profiling;
use crate::task_mapping::{dataset_tasks, get_task_backends, get_task_datasets, Task, TaskMapping};
...
pub struct TaskEvalConfig {
    ...
    pub custom_coref_resolver:
        Option<std::sync::Arc<dyn crate::coref_resolver::CoreferenceResolver>>,
}
...
impl TaskEvaluator {
    ...
    fn evaluate_ner_task(
        ...
        use crate::ner_metrics::evaluate_entities;
            ...
            let all_results: Vec<_> = dataset_data.sentences
                ...
                .map(|(_idx, sentence)| {
                    ...
                    let gold_entities: Vec<Entity> = sentence.entities().iter().map(|g| {
                        ...
                        entity.provenance = Some(anno_core::Provenance::ml("gold", 1.0));
                        entity
                    }).collect();
                        ...
                        if track_per_example {
                            ...
                            let gold: Vec<Entity> = gold_entities
                                ...
                                .map(|g| {
                                    ...
                                    entity.provenance =
                                        Some(anno_core::Provenance::ml("gold", 1.0));
                                    entity
                                })
                                .collect();
                            all_predicted.extend(entities.clone());
    ...
    fn evaluate_coref_task(
        ...
        use crate::backend_factory::create_coref_resolver;
        use crate::coref::entities_to_chains;
        use crate::coref_metrics::CorefEvaluation;
        ...
        let resolver: std::sync::Arc<dyn crate::coref_resolver::CoreferenceResolver> =
...
mod tests {
    ...
    fn test_gliner2_capabilities() {
        let tasks = crate::task_mapping::backend_tasks("gliner2");
