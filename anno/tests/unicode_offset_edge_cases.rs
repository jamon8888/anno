...
use anno::{offset::bytes_to_chars, offset::SpanConverter, Model, StackedNER};
...
fn byte_vs_char_substring_search() {
    proptest::proptest!(|(text in "[\\u{0000}-\\u{FFFF}]{0,200}", pattern in "[\\u{0000}-\\u{FFFF}]{0,50}")| {
...
fn byte_level_utf8_handling() {
    proptest::proptest!(|(bytes in proptest::collection::vec(0u8..=255u8, 0..100))| {
        if let Ok(text) = std::str::from_utf8(&bytes) {
            let ner = StackedNER::default();
            let entities = ner.extract_entities(text, None).unwrap();

            for entity in entities {
                let bstr_view = BStr::new(entity.text.as_bytes());
                prop_assert!(bstr_view.to_str().is_ok(), "Entity text should be valid UTF-8: {:?}", entity.text);
