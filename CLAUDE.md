...
This project is indexed by GitNexus as **anno** (17273 symbols, 40071 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- `cargo-nextest` — profiles `dev-fast`, `ci`, `ml`, `quick` dans `.config/nextest.toml`.
- `cargo-hakari` — workspace-hack pour pré-unifier les features des dépendances partagées (voir crate `workspace-hack/`).

**Target dir** : `E:\cargo-target` (external SSD) — défini en User env var ET dans `.cargo/config.toml [build] target-dir`. Ne jamais pointer vers D:\ (HDD, lld-link hang).
