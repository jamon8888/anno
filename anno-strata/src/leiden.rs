...
impl Leiden {
    ...
    pub fn cluster(&self, graph: &GraphDocument) -> Result<HashMap<String, usize>, String> {
        ...
        // IMPORTANT: Sort node IDs for deterministic initial assignment
        let mut sorted_node_ids: Vec<_> = node_id_map.iter().collect();
        sorted_node_ids.sort_by_key(|(id, _)| id.as_str());

        let mut communities: HashMap<NodeIndex, usize> = HashMap::new();
        for (idx, (_, node_idx)) in sorted_node_ids.iter().enumerate() {
