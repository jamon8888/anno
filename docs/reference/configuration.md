...
| `ner_warmup_model` | `ANNO_RAG_NER_WARMUP_MODEL` | `--ner-warmup-model` | `null` | 0.6 | HF Hub model ID to pre-warm on startup. Default: none |
...
| `tesseract_path` | `ANNO_RAG_TESSERACT_PATH` | `--tesseract-path` | `null` | 0.4 | [DEPRECATED] Legacy path to tesseract binary; ignored by embedded OCR. Default: none |
| `ocr_batch_budget_secs` | `ANNO_RAG_OCR_BATCH_BUDGET_SECS` | `--ocr-batch-budget-secs` | `null` | 0.11 | Per-folder OCR wall-clock budget in seconds. Default: none (unlimited) |
| `ocr_cache_enabled` | `ANNO_RAG_OCR_CACHE_ENABLED` | `--ocr-cache-enabled` | `true` | 0.11 | Enable kreuzberg extraction cache. Default: true |
| `ocr_backend` | `ANNO_RAG_OCR_BACKEND` | `--ocr-backend` | `null` | 0.12 | Primary OCR backend passed to kreuzberg (e.g. paddleocr). Default: none (tesseract) |
...
| `embedder_dtype` | `ANNO_RAG_EMBEDDER_DTYPE` | `--embedder-dtype` | `null` | 0.12 | Embedder weight dtype: f32 (default) or f16 (experimental). Default: none (f32) |
