...
> **Status: SHIPPED 2026-06-23** — all 12 tasks committed on `main` (commits c929ee5d → current HEAD).
...
- [x] **Step 3: Implement the v2 migration**
...
- [x] **Step 2: Add the `conn_lock` helper + module wiring**
...
- [x] **Step 3: Add the delegating methods**
...
- [x] **Step 1: Add a diagnostic test for the sync→search path**
...
- [x] **Step 2: Verify the SQLite FTS path returns hits**
...
- [x] **Step 3 (only if SQLite path insufficient): build LanceDB FTS after sync**
...
- [x] **Step 3: Implement `legal_search_reranked`**
...
- [x] **Step 1: Add the `rerank` field to `LegalSearchParams`**
...
- [x] **Step 4: Route to the reranked path**
...
- [x] **Step 1: Add the dedup field to `AnnoRagServer`**
...
- [x] **Step 2: Convert `legal_ingest_impl` to spawn-and-return**
...
- [x] **Step 3: Write the test (dedup returns same job)**
...
- [x] **Step 1: Find the startup hook**
...
- [x] **Step 4: Knowledge FTS end-to-end check (validates Task 4)**
...
- [x] **Step 1: Surface missing-index as a status, not silent empty**
...
- [x] **Step 3: Rebuild the MCP binary**
...
- [x] **Step 4: Manual MCP feature tour (the acceptance test)**
