...
**Architecture:** Façade-only. The new tools route to handler functions extracted from the existing tools. Three narrow additive read-only helpers on `anno-rag::Store` and `Pipeline` are introduced so the new `sources`/`status`/`forget` can reach the legal corpus state — these are documented exceptions to the "no engine touch" rule, are non-breaking and zero-risk.
...
The spec §10 says "no file in `crates/anno-rag/src/` is modified." That goal is *almost* achievable but `sources()` enumerating legal corpus paths, `status()` reporting legal chunk count, and `forget(path)` cascading to legal chunks all require state that lives behind `Pipeline`/`Store`. This plan introduces **three small additive read-only methods** (Task 1) on `anno-rag` to expose that state. Each method is a thin SELECT or a wrapper around an existing public method. None changes any existing signature, behavior, or test. The plan documents each addition explicitly so the implementer can audit them.

---

## Prerequisite

This plan depends on Phase 1 + Phase 2 being merged (commit `87aeff2b` or later). The 9 legacy tools that will become deprecated must currently exist and pass tests:
...
|------|--------|----------------|
| `crates/anno-rag/src/store.rs` | Modify | Add `list_indexed_folder_paths() -> Result<Vec<String>>` (SELECT DISTINCT) and `count_chunks() -> Result<u64>` (SELECT COUNT). Pure additive readers. |
| `crates/anno-rag/src/pipeline.rs` | Modify | Add `forget_legal_doc_path(&str) -> Result<u64>` — public wrapper around the existing private `self.store.delete_doc_rows()`. Returns rows removed. |
...
- [ ] **Step 5: Add `Pipeline::forget_legal_doc_path` wrapper**

In `crates/anno-rag/src/pipeline.rs`, locate `impl Pipeline` and add (near the other public methods, after `embedder()`):
```rust
    /// Remove all chunk rows whose `source_path` matches `path`.
    /// Thin wrapper around the existing `Store::delete_doc_rows` so the MCP
    /// `forget(path)` tool can cascade to the legal corpus without making
    /// the store field public. Returns the number of rows removed.
    ///
    /// # Errors
    /// Returns LanceDB / Arrow errors propagated from the store layer.
    pub async fn forget_legal_doc_path(&self, path: &str) -> Result<u64> {
        // delete_doc_rows already exists and returns the row count.
        self.store.delete_doc_rows(path).await
    }
```

Check the actual return type of the existing `Store::delete_doc_rows`. If it returns `Result<()>` (no count), adapt the wrapper to count before deleting via `list_indexed_folder_paths` + a chunk query, or simply return `Result<()>` from `forget_legal_doc_path`. The implementer should `grep -n "delete_doc_rows" crates/anno-rag/src/store.rs` and align the wrapper.
...
mod knowledge_phase25_tests {
    ...
    #[tokio::test]
    async fn forget_legal_doc_path_on_empty_store_is_noop() {
        let dir = tempfile::tempdir().expect("temp dir");
        let cfg = AnnoRagConfig {
            data_dir: dir.path().to_path_buf(),
            ..AnnoRagConfig::default()
        ...
        let _ = pipeline.forget_legal_doc_path("C:/does/not/exist").await;
    }
}
```
...
Adds Store::list_indexed_folder_paths, Store::count_chunks, and
Pipeline::forget_legal_doc_path. All three are pure read/passthrough
methods needed by Phase 2.5's new sources()/status()/forget() MCP tools.
...
```rust
    ...
    pub(crate) async fn index_impl_routing(&self, p: IndexParams) -> String {
        ...
        if p.profile == "legal" || p.profile == "all" {
            match self.legal_ingest_impl(crate::LegalIngestParams { path: p.path.clone() }).await {
...
(The implementer should check the exact param type for `legal_ingest_impl` — it likely already takes a struct like `LegalIngestParams { path: String }`. Use the existing type.)
...
In `crates/anno-rag/src/pipeline.rs`, near `forget_legal_doc_path`:
```rust
...
```rust
    #[tool(description = "List all indexed sources: knowledge folders and legal corpus paths. Does not load models.")]
...
```rust
    ...
    #[tokio::test]
    async fn forget_legal_prefix_routes_to_legal_path() {
        let dir = tempfile::tempdir().expect("temp dir");
        let cfg = AnnoRagConfig {
            data_dir: dir.path().to_path_buf(),
            ..AnnoRagConfig::default()
        ...
        let out = server
            .forget_impl_routing(ForgetParams {
                target: "legal:C:/does/not/exist".into(),
            })
            .await;
        let v: serde_json::Value = serde_json::from_str(&out).expect("json");
        assert_eq!(v["ok"], true);
...
impl AnnoRagServer {
    pub(crate) async fn forget_impl_routing(&self, p: ForgetParams) -> String {
        ...
        // Resolution: UUID, legal: prefix, or path.
        if let Some(stripped) = p.target.strip_prefix("legal:") {
            // Legal corpus only.
            match self.pipeline().await {
                Ok(pipeline) => match pipeline.forget_legal_doc_path(stripped).await {
            ...
            // Folder path — cascade both planes, best-effort.
            match self.knowledge_forget_by_path(&p.target).await {
                Ok(n) => knowledge_removed += n,
                Err(e) => errors.push(format!("knowledge forget: {e}")),
            }
            if let Ok(pipeline) = self.pipeline().await {
                match pipeline.forget_legal_doc_path(&p.target).await {
                    Ok(n) => legal_removed += n,
                    Err(e) => errors.push(format!("legal forget: {e}")),
                }
            }
    ...
    /// Forget knowledge sources whose stable_key matches `path`. Helper for the
    /// path-resolution branch of `forget`.
    async fn knowledge_forget_by_path(&self, path: &str) -> std::result::Result<u64, String> {
        let service = self.knowledge().await.map_err(|e| format!("{e}"))?;
        // Use sources() to find a source whose path matches; forget it.
        let sources = service.sources();
        let mut removed = 0;
        for s in &sources {
            if let Some(label) = s.get("label").and_then(|v| v.as_str()) {
                // Knowledge labels are "local:{folder_name}"; the path is in stable_key.
                // KnowledgeService doesn't expose stable_key in sources(), so we
                // match labels conservatively. This is best-effort; an exact path
                // match goes through the legal half via Pipeline::forget_legal_doc_path.
                let _ = label;
            }
        }
        // For Phase 2.5 MVP, path-resolution for the knowledge half is best-effort
        // through the legal cascade; full path→source resolution is roadmap.
        Ok(removed)
    }
}
```

**Note on `knowledge_forget_by_path`:** the `KnowledgeService::sources()` returns labels like `"local:contracts"`, not the full stable_key. Without exposing the stable_key in the sources output (or adding a `find_source_by_path` method), path resolution to the knowledge half is incomplete. This is a known Phase 2.5 limitation: `forget(path)` reliably cascades to legal, and to knowledge **only if the user has the source_id**. Listed in the roadmap.

- [ ] **Step 3: Add `#[tool] forget`**

```rust
    #[tool(description = "Remove an indexed source. Accepts a source_id (UUID), a folder path (cascades best-effort to both planes), or 'legal:{path}'. Does not load models.")]
...
cargo nextest run --package anno-rag-mcp forget_legal_prefix_routes_to_legal_path
git add crates/anno-rag-mcp/src/lib.rs
...
- `forget("legal:C:/path")` cascades to the legal corpus chunks via `Pipeline::forget_legal_doc_path`.
...
- Three small additive read-only methods on `anno-rag` (`Store::list_indexed_folder_paths`, `Store::count_chunks`, `Pipeline::forget_legal_doc_path` + two pass-through accessors) are added. **No existing signature is changed.**
...
Known limitations:
- `forget(folder_path)` cascades reliably to legal (via path match) but knowledge-side path→source resolution is best-effort because `KnowledgeService::sources()` doesn't expose `stable_key`. Documented limitation; full resolution is roadmap.
- `index(profile="legal")` is not idempotent (matches current `legal_ingest`). Description explicitly says so.
