...
**Date:** 2026-06-02
**Status:** Reviewed 2026-06-02 — amend-before-implementation
...
underlying ingestion/search engines. Façade-first: existing implementations are
re-used through thin routing, with only narrow additive helpers for listing and
deleting legal corpus rows and resolving knowledge sources by local path. 9
legacy tools stay functional with deprecated descriptions; the legacy `search`
name is the only intentional surface takeover and must preserve old `rerank`
requests.
...
- **Soft deprecation (D1).** Legacy tool handlers stay intact. Only their
  ...
  replacement. Effective removal is a future phase. The `search` name is
  reused for the unified surface; requests carrying the legacy `rerank`
  parameter route to `legacy_search` for compatibility.
...
- No raw local folder paths in MCP discovery responses. `sources()` returns
  pseudonymous labels/ids only; explicit paths may still be supplied by the
  user to `index(path)` or `forget(target)` because those are user-provided
  command inputs, not server disclosures.
...
```json
{
  "ok": true,
  "sources": [
    { "id": "<uuid>",                    "kind": "knowledge_folder", "label": "local_folder_a1b2c3d4e5f6" },
    { "id": "legal_folder_fedcba654321", "kind": "legal_corpus",     "label": "legal_folder_fedcba654321", "indexed_chunks": 1342 }
  ]
}
```

The `legal_corpus` entries are synthesized from `SELECT DISTINCT folder_path
FROM chunks` in the existing `Store::chunks` table, but the raw path is used
only internally. The visible legal id is `legal_folder_{uuidv5(path)[0..12]}`.
When `forget(id)` receives that id, the server recomputes the same pseudonym
over known folder paths and deletes the matching internal `folder_path`.
...
pub struct ForgetParams {
    /// Knowledge source UUID, legal corpus id from sources(), or an explicit
    /// folder path supplied by the user.
    pub target: String,
}

#[tool(
    description = "Remove an indexed source. Accepts a source_id (UUID), a \
        legal corpus id returned by sources(), or an explicit folder path. \
        Cascades to knowledge SQLite/FTS and (when applicable) to chunks \
        LanceDB. Does not load models."
)]
...
- If it starts with `legal_folder_` → match the id against pseudonymized known
  legal folder paths and route to `Store::delete_folder_rows(folder_path)`
- Otherwise (an explicit folder path supplied by the user) → cascade to both:
  lookup the knowledge source by `stable_key`/`provider_key` matching the path
  and call legal corpus removal for the same path
...
dispatches to the new unified handler. Compatibility rule: if the incoming
`search` payload contains the legacy `rerank` field, the unified handler routes
to `legacy_search_impl` and returns the old response shape. Payloads using
`mode`, `scope`, or `filters` get the new unified behavior.
...
| `forget_path_cascades_both_planes` | `anno-rag-mcp` | no | explicit user path resolution |
| `sources_do_not_expose_raw_local_paths` | `anno-rag-mcp` | no | privacy contract |
...
- `forget("C:/docs")` (an explicit folder path supplied by the user) removes
  both the matching knowledge source and the matching legal chunks in one call.
- `sources()` does not expose raw local paths for either knowledge or legal
  corpora.
