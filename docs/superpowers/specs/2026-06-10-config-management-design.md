...
- Several env vars that should be part of config are handled outside `config.rs`: `ANNO_GDPR_LAYERS` is read in `layers.rs`, `ANNO_ACCELERATOR` in `accelerator.rs`, `ANNO_MODELS_DIR` in `detect.rs`/`embed.rs`, and `ANNO_RAG_MEMORY_NER_MODE` directly in `anno-rag-bin/src/main.rs:146`. None of these are `AnnoRagConfig` fields.
...
| Phase | Scope | Effort |
|-------|-------|--------|
| 0 | **Migrate orphan env vars into `AnnoRagConfig`**: add `gdpr_layers: GdprLayerSet`, `models_dir: Option<PathBuf>` fields; migrate read sites from `layers.rs`, `accelerator.rs`, `detect.rs`, `embed.rs`, `main.rs` | 1 day |
| 1 | Add `syn`/`quote`/`proc-macro2` to `[workspace.dependencies]`, create `anno-config-meta` crate, implement `ConfigMeta` derive (metadata collection only) | 2–3 days |
| 2 | Annotate all `AnnoRagConfig` fields with `#[config_meta(...)]`, add `build.rs` with `generate-schema` feature, commit generated `config-schema.json` / `docs/reference/configuration.md` / `config.toml.example` | 1 day |
| 3 | `AnnoRagConfig::load()` — TOML file + env var loading pipeline; update `anno-rag-bin` and `anno-rag-mcp` to call `load()` | 1 day |
| 4 | `ConfigCliArgs` derive (generates `ConfigOverrides` clap struct); replace `main.rs` manual override chain with `#[command(flatten)]` | 1 day |
| 5 | `anno-rag config` subcommand (init / show / validate) | 1 day |
| 6 | CI job `config-schema-check`; release artifact via cargo-dist ANNO-PATCH sections in `release.yml` | 1 day |

Total: ~8–9 jours de développement.

> **Note cargo-dist:** `release.yml` est autogénéré par cargo-dist 0.32.0. Les modifications doivent être ajoutées dans les sections `# ANNO-PATCH` existantes ou via `dist.toml` (`extra-artifacts`), pas directement dans les scripts `package-*.ps1/sh`.
...
| `ANNO_GDPR_LAYERS` read in `layers.rs` at detection time — moving to config may change semantics | Phase 0 must verify callers: `layers::GdprLayerSet::current()` must read from config, not env, when a config is available; fallback to env for backward compat |
| `ANNO_MODELS_DIR` used by `env_guard.rs` (test helper) with `unsafe set_var` — cannot migrate to config field | Keep `ANNO_MODELS_DIR` as a runtime-only env var, document it separately; do not add to `AnnoRagConfig` |
| `anno-rag-bin` clap does not have `derive` declared locally — relies on workspace inheritance | Workspace clap has `features = ["derive", "env", "wrap_help"]` — confirmed, `#[command(flatten)]` and `#[arg(env)]` will work |
| `syn`/`quote`/`proc-macro2` not in workspace deps — adding them updates `workspace-hack` | Run `cargo hakari generate` after Phase 1 to update `workspace-hack/Cargo.toml` |
| Release workflow autogenerated by cargo-dist 0.32.0 | Extra artifacts must be added via `dist.toml` `extra-artifacts` or ANNO-PATCH sections — not raw script edits |
