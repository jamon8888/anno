...
- Preserve the existing corpus-id path shape, but route legal document-id deletion through `LegalMaintenanceService` so corpus forget does not initialize the model-gated pipeline.
