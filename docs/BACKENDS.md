...
| `gliner2_fastino` | fastino-ai GLiNER2 (NER + classification + structure extraction; Zaratiana 2025). Feature `gliner2-fastino`. Phase 3 multi-session pipeline (8 ONNX graphs: encoder/token_gather/span_rep/schema_gather/count_pred_argmax/count_lstm_fixed/scorer/classifier). Phase 2 structure extraction via `extract_structure(text, &TaskSchema, threshold)` returning `Vec<ExtractedStructure>`. Phase 3.5 `ExecutionMode::IoBinding` keeps tensors device-resident across the 8-session chain (1.5-3× CPU speedup; required for efficient GPU inference) — opt in via `GLiNER2FastinoConfig::default().with_execution_mode(ExecutionMode::IoBinding)`. Runtime LoRA hot-swap is in `gliner2_fastino_candle` (Phase 4). GPU: opt-in via `gliner2-fastino-cuda` / `gliner2-fastino-coreml` features. Issue [#18](https://github.com/arclabs561/anno/issues/18). | Yes | WIP | `SemplificaAI/gliner2-multi-v1-onnx` (also: `fastino/gliner2-multi-v1`, `gliner2-large-v1`, `gliner2-base-v1`) |
| `gliner2_fastino_candle` | fastino-ai GLiNER2 with **PEFT LoRA adapter merge-at-load** (Candle backend). Feature `gliner2-fastino-candle`. Encoder via `candle_transformers::models::debertav2::DebertaV2Model`; 4 head modules (span_rep, count_embed/GRU, count_pred, classifier) ported from PyTorch. Public `load_adapter(name, dir) / unload_adapter() / active_adapter()` API merges PEFT adapters into base weights at load time (~100ms swap, zero per-forward overhead). Use this backend when you serve multiple domain-specific LoRA adapters on one base model (legal/medical/financial). For single-domain or pure-perf: stay on `gliner2_fastino` ONNX. ONNX↔Candle parity verified `max_abs_diff = 5.36e-7` (well below 5e-3 tolerance). GPU: opt-in via `gliner2-fastino-candle-cuda` / `-metal` features. | Yes | experimental | `fastino/gliner2-multi-v1` (PyTorch safetensors, NOT the SemplificaAI ONNX export) |
