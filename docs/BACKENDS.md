...
| `gliner2_fastino` | fastino-ai GLiNER2 (NER + classification; Zaratiana 2025). Feature `gliner2-fastino`. Phase 3 multi-session pipeline (8 ONNX graphs: encoder/token_gather/span_rep/schema_gather/count_pred_argmax/count_lstm_fixed/scorer/classifier). Runtime LoRA hot-swap NOT implemented. GPU: opt-in via `gliner2-fastino-cuda` / `gliner2-fastino-coreml` features. Issue [#18](https://github.com/arclabs561/anno/issues/18). | Yes | WIP | `SemplificaAI/gliner2-multi-v1-onnx` (also: `fastino/gliner2-multi-v1`, `gliner2-large-v1`, `gliner2-base-v1`) |
