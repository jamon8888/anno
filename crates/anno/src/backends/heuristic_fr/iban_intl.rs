...
pub fn extract_iban_intl(text: &str) -> Vec<Entity> {
    candidate_re().find_iter(text)
        .filter(|m| iban_mod97(m.as_str()))
        .map(|m| {
            let start = text[..m.start()].chars().count();
            let end = text[..m.end()].chars().count();
            Entity::builder(m.as_str(), EntityType::Custom { name: "iban".into(), category: EntityCategory::Numeric })
                .span(start, end)
                .confidence(1.0_f32)
                .build()
        }).collect()
...
fn iban_mod97(raw: &str) -> bool {
    ...
    for c in rearranged.chars() {
        ...
        else { numeric.push_str(&((c as u32 - 'A' as u32 + 10).to_string())); }
    }
    let mut r: u32 = 0;
    for d in numeric.chars() { r = (r * 10 + d.to_digit(10).unwrap()) % 97; }
    r == 1
}
