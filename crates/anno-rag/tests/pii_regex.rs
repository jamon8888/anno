...
fn load_baseline_recall() -> HashMap<String, f64> {
    let path = pii_corpus_dir().parent().expect("pii corpus parent dir").join("pii_baseline.toml");
...
fn regex_pii_recall_meets_baseline() {
    ...
    for cat in REGEX_CATEGORIES {
        ...
        eprintln!("{cat}: recall={recall:.4} precision={precision:.4} (baseline recall {base})");
        if recall < base * 0.98 {
