...
mod tests {
    ...
    fn ent(v: &str) -> DetectedEntity {
        DetectedEntity { original: v.to_string(), start: 0, end: v.len(), category: EntityCategory::Custom("email_address".into()), confidence: 0.9, source: DetectionSource::Ner }
    }

    #[test]
    fn accepts_normal() { assert_eq!(EmailRfcValidator.validate(&ent("alice@example.com"), ""), ValidationResult::Accept); }

    #[test]
    fn accepts_with_tag() { assert_eq!(EmailRfcValidator.validate(&ent("alice+tag@sub.example.fr"), ""), ValidationResult::Accept); }

    #[test]
    fn rejects_no_at() { assert!(matches!(EmailRfcValidator.validate(&ent("plainstring"), ""), ValidationResult::Reject { reason: "email_no_at" })); }

    #[test]
    fn rejects_no_dot_in_domain() { assert!(matches!(EmailRfcValidator.validate(&ent("a@localhost"), ""), ValidationResult::Reject { reason: "email_domain_invalid" })); }

    #[test]
    fn rejects_domain_starts_with_dot() { assert!(matches!(EmailRfcValidator.validate(&ent("a@.example.com"), ""), ValidationResult::Reject { .. })); }
}
