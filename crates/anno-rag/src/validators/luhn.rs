...
mod tests {
    ...
    fn luhn_accepts_known_valid_siret() {
        ...
        assert_eq!(LuhnValidator::new("SIRET").validate(&e, ""), ValidationResult::Accept);
    }

    #[test]
    fn luhn_rejects_random_14_digit_run() {
        let e = entity_with("12345678901234");
        assert_eq!(LuhnValidator::new("SIRET").validate(&e, ""), ValidationResult::Reject { reason: "luhn_failed" });
    }
    ...
    fn luhn_accepts_valid_card_number() {
        ...
        assert_eq!(LuhnValidator::new("card_number").validate(&e, ""), ValidationResult::Accept);
    }
}
