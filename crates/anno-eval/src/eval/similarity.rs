...
pub fn cluster_embeddings(
    ...
    let points: Vec<Vec<f32>> = index
        .embeddings
        .iter()
        .map(|e| e.embedding.to_vec())
        .collect();
    let kmeans = clump::Kmeans::new(k)
        .with_max_iter(max_iterations.max(1))
        .with_tol(1e-4)
        .with_seed(42);
    let res = match kmeans.fit(&points) {
        Ok(r) => r,
        Err(_) => {
