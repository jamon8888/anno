...
fn test_single_letters_not_persons() {
    ...
    let test_cases = vec![
        ("A, B, C are variables", vec![]), // Should extract nothing
        ("The letter A is important", vec![]), // Should extract nothing
        ("Options: A, B, or C", vec![]), // Should extract nothing
        ("Choose A or B", vec![]), // Should extract nothing
    ];
