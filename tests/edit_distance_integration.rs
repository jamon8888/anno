...
fn test_edit_distance_basic_workflow() {
    ...
    assert!(
        similarity > 0.7,
        "Similarity should be high for typo: {}",
        similarity
    );
}
...
fn test_generator_with_edit_distance_metric() {
    ...
    if let Some(e) = einstein {
        assert!(
            e.string_sim > 0.7,
            "String similarity should be high: {}",
            e.string_sim
        );
    }
}
...
fn test_generator_with_wildcard_metric() {
    ...
    let curie = candidates
        .iter()
        .find(|c| c.label.to_lowercase().contains("curie"));
...
fn test_similarity_metric_from_str() {
    ...
    assert_eq!(SimilarityMetric::parse_str("invalid"), None);
}
...
fn test_candidate_generation_performance() {
    ...
    let queries = vec!["einstein", "curie", "newton", "darwin", "tesla"];
