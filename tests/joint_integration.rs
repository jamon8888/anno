...
fn test_joint_model_coreference_chain() {
    ...
    for chain in &result.chains {
        println!("  Chain {}: {:?}", chain.cluster_id.unwrap_or(0), chain.mentions);
    }
}
...
fn test_joint_model_pronoun_resolution() {
    ...
    let entities = vec![
        ...
        Entity::new("Nobel Prize", EntityType::Other("AWARD".to_string()), 18, 29, 0.85),
...
fn test_belief_propagation_convergence() {
    ...
    for var in &variables {
        ...
        assert_eq!(argmax, Some(0), "Expected Person (index 0) to have highest probability");
    }
}
...
fn test_belief_propagation_with_binary_factors() {
    ...
    let factors: Vec<Box<dyn Factor>> = vec![
        ...
        Box::new(UnaryCorefFactor::new(
            1,
            vec![(AntecedentValue::Mention(0), 2.0), (AntecedentValue::NewCluster, 0.0)],
        )),
    ];
...
fn test_link_ner_factor_with_knowledge() {
    ...
    let factor = LinkNerFactor::new(0, LinkNerWeights::default()).with_knowledge(Arc::new(knowledge));
...
fn test_joint_model_many_mentions() {
    ...
    for i in 0..20 {
        ...
        text.push_str(&name);
        text.push(' ');
        entities.push(Entity::new(&name, EntityType::Person, offset, offset + name.len(), 0.9));
        offset += name.len() + 1;
    }
